...
from severity_rules import get_severity

app = Flask(__name__)
CORS(app)  # Enable Cross-Origin Resource Sharing

# Configure upload folder and limit file size to 5MB
UPLOAD_FOLDER = os.path.join(project_dir, "outputs")
os.makedirs(UPLOAD_FOLDER, exist_ok=True)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['MAX_CONTENT_LENGTH'] = 5 * 1024 * 1024  # 5MB Max size limit

# Paths
model_path = os.path.join(project_dir, "models", "agro_disease_model.h5")
tokenizer_path = os.path.join(project_dir, "outputs", "tokenizer.pkl")
class_names_path = os.path.join(base_dir, "class_names.json")
class_info_path = os.path.join(base_dir, "class_info.json")
treatment_db_path = os.path.join(base_dir, "api", "treatment_db.json")
